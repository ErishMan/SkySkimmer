...
class TestRequiredFields:
    def test_valid_minimal_config_loads(self):
        settings = Settings(_env_file=None, **MINIMAL_VALID_ENV)
    ...
    def test_missing_tequila_key_raises(self):
        with pytest.raises((ValidationError, SystemExit)):
            Settings(_env_file=None, discord_webhook_url="https://discord.com/api/webhooks/123/abc")

    def test_missing_discord_webhook_raises(self):
        with pytest.raises((ValidationError, SystemExit)):
            Settings(_env_file=None, tequila_api_key="test-key")
...
class TestOptionalFields:
    ...
    def test_poll_interval_too_low_raises(self):
        with pytest.raises(ValidationError):
            Settings(_env_file=None, **MINIMAL_VALID_ENV, poll_interval_minutes=2)

    def test_poll_interval_too_high_raises(self):
        with pytest.raises(ValidationError):
            Settings(_env_file=None, **MINIMAL_VALID_ENV, poll_interval_minutes=9999)
...
class TestSupabasePairing:
    def test_supabase_url_without_key_raises(self):
        with pytest.raises(ValidationError, match="must both be set"):
            Settings(_env_file=None, **MINIMAL_VALID_ENV, supabase_url="https://xyz.supabase.co")

    def test_supabase_key_without_url_raises(self):
        with pytest.raises(ValidationError, match="must both be set"):
            Settings(_env_file=None, **MINIMAL_VALID_ENV, supabase_key="super-secret-key")
...
class TestEffectiveErrorWebhook:
    ...
    def test_uses_dedicated_error_webhook_when_present(self):
        ...
        settings = Settings(_env_file=None, **MINIMAL_VALID_ENV, discord_error_webhook_url=error_url)
